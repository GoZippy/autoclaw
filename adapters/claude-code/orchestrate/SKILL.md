---
name: orchestrate
description: Multi-agent parallel development orchestrator. Reads task manifests, builds dependency DAGs, generates sprint plans, assigns scoped work to parallel agents, and coordinates review gates.
---
...
Agents may be dispatched using the `Agent` tool for parallel execution. Each agent receives a self-contained prompt with its task list, scope constraints, and branch name.
