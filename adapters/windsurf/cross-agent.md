...
- **Idempotency** — read each inbox message once. On first read, record it (write `inboxes/windsurf/_state/<msg-id>.json`), act on it, then move the file to `processed/`. Never re-process a message already handled.
