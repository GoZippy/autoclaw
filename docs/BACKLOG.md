...
| HKS-4..5 | Hook actions launch_skill / spawn_runner / relay (cross-machine wake) + non-message event sources (heartbeat_stall, claim_stale, consensus, autobuild_fail) | spec §Sequencing 4–5 | **done** (2026-06-13) — `hookEvents.ts`/`hookBus.ts` leaf modules; 3 actions wired in extension; stall/claim tick + consensus(bridge)/autobuild_fail(autobuild) emits |
